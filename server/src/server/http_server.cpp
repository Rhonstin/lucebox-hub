...
bool HttpServer::route_request(int fd, const HttpRequest & hr) {
    ...
    // prefills the compressed prompt. (This subsumes main's
    // admit-oversized-when-compressible gate and additionally clamps
    // max_tokens for fixed-budget clients like Letta; tool requests bypass
    // compression so they are still validated up front.)
